...
export class ToolRestrictionService extends Service {
    ...
    async seedBoardsFast() {
        ...
        if (presets !== undefined) {
            for (const preset of await presets.list()) {
                if (boards[preset.id] !== undefined)
                    continue;
                const authored = await this.authoredBoard(preset);
                if (authored !== undefined)
                    boards[preset.id] = authored;
            }
        }
        await this.commitBoards(boards);
    }
    ...
    async commitBoards(boards) {
        ...
        for (const [presetId, board] of Object.entries(previous)) {
            if (boards[presetId] === undefined && board.length > 0)
        ...
        const widest = [...Object.values(boards)].sort((left, right) => right.reduce((sum, group) => sum + group.tools.length, 0)
            - left.reduce((sum, group) => sum + group.tools.length, 0))[0];
        const fallback = widest ?? buildGroups(this.ctx.tools.schemas());
        if (fallback.length === 0 && Object.keys(boards).length === 0)
            return;
        if (fallback.length > 0)
            boards.default = fallback;
        try {
