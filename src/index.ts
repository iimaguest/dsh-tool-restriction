...
export const TOOL_RESTRICTION_SETTINGS_NAMESPACE = 'tool-restriction' as SettingsNamespace
 ...
 */
const optionalMaskShape = maskShape.default(undefined as unknown as { allow: string[] })
 ...
 */
export function resolveSessionToolRestriction(events: readonly SessionEvent[]): ToolMask | undefined {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index] as SessionEvent
    if (event.type === 'tool-restriction/selected') return event.data.mask
  }
  return undefined
}
...
interface AppliedRestriction {
  ...
  /** Dispose the agent-scoped monotonic guard. */
  guard: () => void
}
...
export class ToolRestrictionService extends Service {
  ...
  private readonly deploymentDefault: ToolMask | undefined
  /** Live settings source (composition base while no settings provider mounts). */
  private settingsSource: () => ToolRestrictionSettings

  /** Per-agent applied restriction/guard state, keyed by the agent. */
  private readonly applies = new Map<Agent, AppliedRestriction>()
  ...
  constructor(ctx: Context, config: Config) {
    ...
    // The settings section's per-preset boards are derived from each preset's
    // composed tool set. Composing a preset mounts its whole plugin tree
    // (`agentPresets.standingKeyFor`), which is far too expensive to run in
    // this plugin's activation: it would hold Loader settlement hostage while
    // every roster preset composes. So the boot path seeds the fast sources
    // only — standing mounts already composed by a live session, authored
    // `tools.yml`, and the global tool view — and the full composition pass
    // runs only on recomposition events (`tools/change`, `agent/created`),
    // which fire exactly when a preset is actually composed or edited.
    void this.seedBoardsFast()
    // A `tools/change` fires while Loader settlement is still in flight — a
    // sibling plugin registering a tool during its own `apply()` is enough —
    // and the full composition pass mounts every roster preset, whose own
    // tool registrations fire more `tools/change`, re-entering this handler.
    // Composing mid-settlement therefore holds Loader settlement hostage to
    // the whole preset cascade. The event handlers only ever schedule the
    // pass; the deferred runner skips until settlement, so boot composes
    // nothing and the recomposition events that do matter (a preset actually
    // composed or edited, an agent created) still refresh the boards once.
    ctx.on('tools/change', () => { this.scheduleBoardsRefresh() })
    ...
    ctx.on('agent/created', () => { this.scheduleBoardsRefresh() })
    ...
    for (const session of ctx.sessions.list()) { this.seedSession(session) }
    ...
    for (const agent of ctx.agents.list()) { this.installForAgent(agent) }
   ...
   */
  private installForAgent(agent: Agent): void {
    if (this.applies.has(agent)) return
    ...
    const guard = agent.ctx.tools.guard((execution) => {
      ...
      if (current.allow.includes(execution.name)) return undefined
      ...
      if (this.ctx.tools.get(execution.name, agent) !== undefined) return undefined
  ...
  private uninstallForAgent(agent: Agent): void {
    const applied = this.applies.get(agent)
   ...
   * one pending pass.
   */
  private scheduleBoardsRefresh(): void {
    if (this.refreshPending) return
    this.refreshPending = true
    const loader = this.ctx.get('loader')
    const deferred = loader === undefined ? Promise.resolve() : loader.await()
    // The pending flag stays set across the whole pass (not just until the
    // deferred start) so a `tools/change` that the pass itself fires — its
    // preset mounts register tools — cannot re-enter and queue a cascade.
    void deferred
      .then(() => this.refreshBoards())
      .catch((error: unknown) => {
        this.ctx.logger.warn(`dsh-tool-restriction: deferred settings board refresh failed: ${String(error)}`)
      })
  ...
  private async refreshBoards(): Promise<void> {
    ...
    await this.commitBoards(boards)
  }
   ...
   */
  private async seedBoardsFast(): Promise<void> {
    const settings = this.ctx.get('settings')
    if (settings === undefined) return
    const boards: Record<string, readonly ToolRestrictionGroup[]> = {}
    // Already-composed standing mounts are the authoritative per-preset board
    // and cost nothing to read: the composition exists, so the scope's tools
    // are visible immediately.
    for (const mount of livePresetMounts()) {
      if (mount.key === undefined) continue
      const board = buildGroups(this.ctx.tools.schemas(mount.key))
      if (board.length > 0) boards[mount.presetId] = board
    }
    // Presets with an authored `tools.yml` seed their board from the file
    // without composing anything.
    const presets = this.ctx.get('agentPresets')
    if (presets !== undefined) {
      for (const preset of await presets.list()) {
        if (boards[preset.id] !== undefined) continue
        const authored = await this.authoredBoard(preset)
        if (authored !== undefined) boards[preset.id] = authored
      }
    }
    await this.commitBoards(boards)
  }
   ...
   */
  private async authoredBoard(preset: AgentPreset): Promise<readonly ToolRestrictionGroup[] | undefined> {
    const presets = this.ctx.get('agentPresets')
    if (presets === undefined) return undefined
    try {
      const resolved = await presets.resolve(preset.id)
      const tools = await readPresetTools(dirname(resolved.path))
      const groups = tools?.groups
      if (groups !== undefined && Object.keys(groups).length > 0) {
        const rows = Object.entries(groups).flatMap(([group, names]) => {
          const row = names
            .filter(name => name !== RUN_CODE_NAME)
            .map(name => ({ name, description: '' }))
          return row.length === 0 ? [] : [{ group, tools: row }]
      ...
      // A broken preset or a stale roster id yields no authored board.
      return undefined
    }
  }
   ...
   */
  private applyMask(agent: Agent, mask: ToolMask | undefined): void {
    const applied = this.applies.get(agent)
    if (applied === undefined) return
    if (mask !== undefined) {
  ...
  private seedSession(session: Session): void {
    if (hasSelection(session.snapshotEvents())) return
    const presetId = this.ctx.sessionProjections.stateOf(session, 'agentPreset') ?? undefined
    ...
    if (presetId === undefined) {
      if (deploymentDefault !== undefined) this.materialize(session, deploymentDefault)
      return
    }
   ...
   */
  private async seedPresetDefault(
    session: Session,
    presetId: string,
    deploymentDefault: ToolMask | undefined,
  ): Promise<void> {
    if (hasSelection(session.snapshotEvents())) return
    const presets = this.ctx.get('agentPresets')
   ...
   */
  private filterGuidance(assembly: PromptAssembly, agent: Agent | undefined): PromptAssembly {
    if (agent === undefined) return assembly
    const mask = resolveSessionToolRestriction(agent.session.snapshotEvents())
    if (mask === undefined) return assembly
    const tools = this.ctx.tools
    const sections = assembly.sections.filter((section) => {
      if (!TOOL_GUIDANCE_SECTION.test(section.name)) return true
      const name = section.name.slice(TOOL_GUIDANCE_PREFIX.length)
      if (mask.allow.includes(name)) return true
  ...
  private async persistPresetPreference(agent: Agent, mask: ToolMask | undefined): Promise<void> {
    const settings = this.ctx.get('settings')
    if (settings === undefined) return
    const presetId = this.ctx.sessionProjections.stateOf(agent.session, 'agentPreset') ?? undefined
