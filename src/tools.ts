 ...
 */
export async function readPresetTools(directory: string): Promise<PresetTools | undefined> {
  let raw: string
  try {
  ...
  const toolNames = (value: unknown): readonly string[] | undefined => {
  ...
  const defaultAllow = declaredDefault === undefined
    ...
    : typeof declaredDefault === 'object' && declaredDefault !== null
      ...
      ? toolNames((declaredDefault as Record<string, unknown>)['allow'])
      : undefined
  ...
  const finish = (value: PresetTools): PresetTools | undefined =>
  ...
  if (typeof groupsRecord === 'object' && groupsRecord !== null && !Array.isArray(groupsRecord)) {
    ...
    for (const [name, value] of Object.entries(groupsRecord as Record<string, unknown>)) {
      if (name === 'order') continue
      const names = toolNames(value)
      if (names !== undefined) groups[name] = names
    }
    // A present `order` is honoured even with no groups: the picker may map it
    // onto host-declared group names.
    const order = (groupsRecord as Record<string, unknown>)['order']
 ...
 */
export function renderPresetTools(tools: PresetTools): string | undefined {
 ...
 * @param tools - the tool board to store.
 */
export async function writePresetTools(directory: string, tools: PresetTools): Promise<void> {
